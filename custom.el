(custom-set-variables
 ...
 '(package-selected-packages
   '(cider consult corfu diff-hl diminish eat edit-indirect editorconfig ef-themes
           eglot-java eglot-tempel fish-mode flymake-kondor geiser-guile gptel
           jinx js-comint just-mode magit marginalia markdown-mode modus-themes
           move-text olivetti orderless paredit pcmpl-args plantuml-mode
           rainbow-delimiters rg typescript-mode undo-tree vertico which-key))
     ...
     (eval set 'geiser-repl-startup-hook
           (let*
               ((dir (dir-locals-find-file "."))
                (full-path
                 (expand-file-name "defs.scm" (if (stringp dir) dir (car dir)))))
             (lambda nil (geiser-load-file full-path))))
     (eval defun user/game-repl nil "Starts the game with repl" (interactive)
           (let*
